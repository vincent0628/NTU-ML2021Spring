...
if __name__ == '__main__':       
    ...
    **The only diffference with HW3 is that you should use loss in  knowledge distillation.**
    ...
    optimizer = torch.optim.AdamW(student_net.parameters(), lr=0.0005)
    ...
    for epoch in range(n_epochs):
        if epoch == 150:
            optimizer = torch.optim.AdamW(student_net.parameters(), lr=0.0001)
        if epoch == 230:
            optimizer = torch.optim.SGD(student_net.parameters(), lr=0.0001, momentum=0.9)
        if epoch == 300:
            optimizer = torch.optim.SGD(student_net.parameters(), lr=0.00001, momentum=0.9)
        # ---------- Training ----------
        ...
        if valid_acc>0.82:
          torch.save(student_net.state_dict(), f'student_net_{epoch}_{valid_acc:.5f}.bin')
    ...
    student_net.load_state_dict(torch.load('student_net_finish.bin'))
    student_net.eval()
